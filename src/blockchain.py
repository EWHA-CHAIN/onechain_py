...
def generateNextBlock(blockData):
    ...
    newBlockHeader = findBlock(currentVersion, nextIndex, previousHash, 
    nextTimestamp, merkleRoot, difficulty)
...
def findBlock(currentVersion, nextIndex, previoushash, nextTimestamp, merkleRoot, difficulty):
    nonce = 0
    while(True):
        hash = calculateHash(currentVersion, nextIndex, previoushash, nextTimestamp, 
        merkleRoot, difficulty, nonce)
        if hashMatchesDifficulty(hash, difficulty):
            return BlockHeader(currentVersion, nextIndex, previoushash, nextTimestamp, 
            merkleRoot, difficulty, nonce)
        nonce += 1 
...
def getDifficulty(aBlockchain):
    ...
    if ((latestBlock.header.index % DIFFICULTY_ADJUSTMENT_INTERVAL == 0) 
    and (latestBlock.header.index != 0)) is True:
...
def isValidTimestamp(newBlock, previousBlock):
    ...
    and newBlock.header.timestamp-60 < ut.getCurrentTimestamp()
...
def isValidNewBlock(newBlock, previousBlock):
    if isValidBlockStructure(newBlock) is False:
        ...
        return False
    elif  (newBlock.data.length != 0 and (merkle("sha256").sync(newBlock.data).root() 
    != newBlock.header.merkleRoot)) or (len(newBlock.data) == 0 
    and (np.repeat('0', 64)) != newBlock.header.merkleRoot))
...
def isValidChain(blockchainToValidate):
    if json.dumps(blockchainToValidate[0]) != json.dumps(genesisBlock()): 
        #json 라이브러리의 dumps 함수 사용해서 json형식으로 바꿔줌. 
        return False
